...
describe('#breedDetailsFromFile', () => {
  ...
  it('provides, via callback, undefined for a breed that does not exist', (done) => {
    breedDetailsFromFile('Sapphire', (desc) => {
      assert.equal(undefined, desc);
      done(); 
    });      
  });

  it('provides, via callback, undefined for an empty breed name', (done) => {
    breedDetailsFromFile('', (desc) => {
      assert.equal(undefined, desc);
      done(); 
    });      
  });

  it('provides, via callback, undefined for a null breed name', (done) => {
    breedDetailsFromFile(null, (desc) => {
      assert.equal(undefined, desc);
      done(); 
    });      
  });
